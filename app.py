# -*- coding: utf-8 -*-
# Crypto Daily Dashboard – Core Portfolio (One File, Streamlit)
# Compatible con Python 3.9+ (usa typing.Optional en vez de |)
...
YELLOW_BUFFER_PCT = 10                  # "cerca" del target (±10%)
...
def get_btc_price(price_map: dict, manual: Optional[float] = None) -> Optional[float]:
...
def compute_mode(price_map: dict, threshold=0.10):
...
# --------- Macro quick & Derivados (opcionales, robustos) ----------
...
# -------------------- UI --------------------
st.markdown("### 🗓️ Reporte Diario – Core Portfolio")
...
# Señales
signals, totals = compute_signals(df, prices, TARGETS, btc_price or 1.0, [tr1,tr2,tr3], split_btc, yb)
...
# Derivados
st.markdown("### 🔗 Derivados (Binance futures proxies)")
...
if not oi_df.empty:
    fig_oi = px.line(oi_df, x="timestamp", y="sumOpenInterest", title="BTC Open Interest (1h)")
    c11.plotly_chart(fig_oi, use_container_width=True)
else:
    c11.info("Sin datos de OI disponibles ahora.")
...
st.success("Listo. Semáforos activos, TP por tramos y DCA planner. Ejecuta sin emociones. 🚀")
